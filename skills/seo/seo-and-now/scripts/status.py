...
def split_cells(line):
    ...
    Duplicated from seo-setup/scripts/scaffold.py on purpose: each skill stays standalone."""
...
def decide(s, today):
    ...
    c = s["connections"]
    if not s["config"]:
        return "setup", ["`seo-setup`: fill config.md (canonical host, framework, sitemap, brand_regex, CTR calibration)"], then
    if not c["GSC_PROPERTY"] or not c["SITEMAP_SUBMITTED_AT"]:
        now.append("`seo-connect`: Search Console property and sitemap submission are not recorded in connections.md "
                   "(site not live yet: `seo-grill` first)")
    if not s["strategy"] or not s["glossary"]:
        now.append("`seo-grill`: strategy.md and glossary.md are still the template")
    if now:
        return "setup", now, then
    a = s["audit"]
    if a is None:
        return "audit", ["`seo-tech-audit --crawl`: no audits/*.json yet; run until zero FAIL, then the launch checklist"], then
    stage = "loop"
    if a["fail"]:
        stage = "audit"
        now.append(f"`seo-tech-audit`: {a['file']} still has {a['fail']} FAIL ({', '.join(a['fail_ids'])}); "
    ...
    if s["due"]:
        now.append(f"`seo-gsc-review`: {len(s['due'])} row(s) past their verify date need a verdict "
    ...
    if exp is None:
        ...
        if exp.name not in s["log_source"]:
            now.append(f"`seo-gsc-review` on exports/{exp.name}: not named in any log's Source line yet")
        elif age > EXPORT_MAX_AGE:
            now.append(f"export Search Console again (exports/{exp.name} is {age} days old), then `seo-gsc-review`")
    for r in other_todo:
        skill = {"content": "seo-content", "links": "seo-links", "distribution": "seo-distribution",
                 "diagnose": "seo-diagnose"}.get(r.get("bucket"), "apply per seo-gsc-review actions.md")
    ...
    for slug in s["briefs"]:
        if slug not in s["drafts"]:
            now.append(f"`seo-content`: briefs/{slug}.md has no draft yet")
    for slug in s["drafts"]:
        shipped = any(slug in r.get("url", "") and r.get("bucket") == "content" and r.get("status") != "todo"
                      for r in s["rows"])
        if not shipped:
            now.append(f"`seo-review` on drafts/{slug}.md, then ship it, request indexing, and log a `content` row")
    for r in s["rows"]:
        if r.get("bucket") != "content" or r.get("status") not in ("applied", "verify", "won", "no-change"):
            continue
        url = r.get("url", "")
        for bucket, skill in (("links", "seo-links"), ("distribution", "seo-distribution")):
    ...
    if not now:
        now.append("nothing open this week; next export and `seo-gsc-review` next week")
    if s["next_verify"]:
        then.append(f"{s['next_verify'].isoformat()}: first verify date reached; `seo-gsc-review` grades those rows")
...
def main():
    ...
    if not root.is_dir():
        print(f"no workspace at {root}: run `seo-setup` first")
        sys.exit(2)
    domains = a.domains or sorted(p.name for p in root.iterdir() if p.is_dir() and (p / "config.md").exists())
    if not domains:
        print(f"no domain folder under {root}: run `seo-setup <domain>` first")
        sys.exit(2)
    reports = []
    for d in domains:
        base = root / d
        if not base.is_dir():
            print(f"no folder {base}: run `seo-setup {d}` first")
            sys.exit(2)
