...
Every skill reads and writes `docs/seo/<domain>/` in the site's repository: `config.md` (site facts), `connections.md` (what is connected), `strategy.md` and `glossary.md` (from the interview), `log/` (every action and its outcome, one file per week), `briefs/`, `drafts/`, `exports/`. Layout and log format: `docs/seo/README.md`, written by `seo-setup`. No workspace yet: `seo-setup` first.

## Flows

**New site or domain**, in this order:

1. `seo-setup`: workspace, `config.md`, pointer block in `AGENTS.md` and `CLAUDE.md`.
2. `seo-connect`: wizard for Search Console, sitemap, Bing, IndexNow.
...
| Write or refresh a page | `seo-content` | agent or you |
| Is this draft right for the query and clean on the page? | `seo-review` | agent or you |
| Why did clicks, impressions, or position drop? | `seo-diagnose` | agent or you |
| Get links | `seo-links` | agent or you |
| Promote a page | `seo-distribution` | agent or you |
