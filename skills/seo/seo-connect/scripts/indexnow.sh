...
#
# Reads INDEXNOW_KEY_FILE from docs/seo/<domain>/connections.md (written by the seo-connect
...
[[ -f "$connections" ]] || { echo "no $connections: run seo-connect first" >&2; exit 1; }
key_file=$(sed -n 's/^- *INDEXNOW_KEY_FILE: *\([^ ]*\).*/\1/p' "$connections" | head -1)
[[ -n "$key_file" ]] || { echo "INDEXNOW_KEY_FILE is empty in $connections: run the seo-connect wizard, IndexNow stage" >&2; exit 1; }
...
body=$(curl -s --max-time 15 -A "seo-connect indexnow.sh" -w '\n%{http_code}' "$key_file" || printf '\n000')
status=${body##*$'\n'}; body=${body%$'\n'*}
...
code=$(curl -s -o /dev/null --max-time 30 -w '%{http_code}' -X POST \
  -H 'Content-Type: application/json; charset=utf-8' -A "seo-connect indexnow.sh" \
