...
2. **Map every FAIL and WARN to a fix** with [references/fixes.md](references/fixes.md), keyed by check id. Inside the site's repository, apply the fix in the layout or template that emits the `<head>` instead of describing it: `head_template` from `config.md`, or grep for `<title`, `rel="canonical"`, `sitemap`, `robots`. Site not in the repository (`framework` is a hosted CMS, `head_template` blank): write each fix as a step in the CMS admin, with the replacement URL or text filled in, using the CMS section of fixes.md; the user applies, you verify.
