...
- Check: search the query in the target market; compare the CTR-gap bucket in the last `seo-gsc-review`; the Generative AI performance report for AI Overview impressions.
- Fix: title and meta for a reason to click, a direct answer in the first paragraph to be the cited source; accept the ceiling. `seo-gsc-review` actions.md, CTR gap.
...
- Fix: no quick edit exists. Spam or site-reputation issues: remove the violating content, then Request review. Core update: the pages that lost are compared against the helpful-content questions and improved through `seo-content` refreshes; recovery is measured over the next update, not the next week.
...
- Check: `seo-tech-audit` on three affected URLs (render, noindex, canonical, redirect, soft 404, hosts); URL Inspection live test; `git log --since=<drop date>` on `head_template`, `robots_txt`, the sitemap generator, redirects and routing config; Crawl stats for 5xx and response time.
...
- Check: git history of the page and of navigation and templates; the cannibalization bucket in `seo-gsc-review` (needs page×query data); internal link count to the page before and after (the audit's crawl mode).
...
- Fix: the decay recipe in `seo-gsc-review` actions.md: material update, new section, fresh evidence, date moved only after the body changed; via `seo-content` refresh.
