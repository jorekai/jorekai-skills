...
5. **One change, one row.** Apply the fix the confirmed hypothesis names (in hypotheses.md), write a `diagnose` row to the week's log with `verify after` 28 days out (`scaffold.py <domain> --log` in `seo-setup` gives path and id), and stop. A second change before the verify date makes the outcome unreadable.
